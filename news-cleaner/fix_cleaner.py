...
def clean_news(news_list):
    ...
    for news in news_list:
        try:
            ...
            if len(content) < MIN_CONTENT_LENGTH:
                logger.info(f"짧은 컨텐츠 필터링: {title}")
                continue
    ...
    logger.info(f"정제 완료: {len(cleaned_list)}개 뉴스 (제외: {len(news_list) - len(cleaned_list)}개)")
    return cleaned_list
