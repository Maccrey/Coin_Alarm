    ...
    for news in news_list:
        try:
            ...
            if len(content) < MIN_CONTENT_LENGTH:
                logger.info(f"짧은 내용 필터링: {title}")
                continue
            
            # 광고성 컨텐츠 필터링
            if is_ad_content(title, content):
                logger.info(f"광고성 필터링: {title}")
                continue
            ...
            cleaned_news = {
                ...
                'news_hash': news_hash  # 해시값 추가
            }
            
            cleaned_list.append(cleaned_news)
            logger.info(f"정제 완료: {title}")
            
        except Exception as e:
            logger.error(f"뉴스 정제 오류: {e}")
            continue
    
    logger.info(f"정제 완료: {len(cleaned_list)}개 뉴스 (원본: {len(news_list)}개)")
    return cleaned_list
